...
Original file is located at
    https://colab.research.google.com/drive/1zWKONG0uJQVp4iJLRXnSpKdZHcHZbbQu
...
def get_mean_sepal_length_from_class_top_n_width(iris, flower_class:str, n:int):
    ...
    '''
    # STUDENT TODO START:
    data = iris.iloc[np.where(iris["species"] == flower_class)[0]]
    top_n = data.nlargest(n, 'sepal_width')
    return top_n['sepal_length'].mean()
...
def string_to_categorical_int(iris, n : int):
    ...
    # STUDENT TODO START:
    import pandas as pd
    df2 = iris.groupby('species').head(n)
    mapping = {category : i for i, category in enumerate(df2['species'].unique())}
...
for v in df['Volume'].unique():
    subset = df[df['Volume'] == v]
    plot.scatter(subset['Time Deltas'], subset['Declination'], alpha=0.25, label=f'Mars Position (Volume {v})', marker='+')

plot.title("Tycho Brahe's Mars Data")
plot.xlabel("Time since first observation (days)")
plot.ylabel("Angle of Declination (degrees)")
